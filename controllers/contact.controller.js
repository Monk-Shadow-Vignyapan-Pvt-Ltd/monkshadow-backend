...
export const addContact = async (req, res) => {
  try {
    ...
    const { name, phone, email, message,companyName,pageName,websiteUrl, isContactClose,showForAll,status,followupMessage } = req.body;
    ...
    if (existingContact) {
      ...
      existingContact.followupMessage = followupMessage;
      await existingContact.save();
      return res.status(200).json({ message: "Contact updated successfully", contact: existingContact, success: true });
    }

    // Create a new contact document
    const newContact = new Contact({ name, phone, email, message,companyName,pageName,websiteUrl, isContactClose,showForAll,status,followupMessage });
...
export const updateContact = async (req, res) => {
  try {
    ...
    const updatedData = { name, phone, email, message,companyName,pageName,websiteUrl, isContactClose,showForAll,status,followupMessage };
